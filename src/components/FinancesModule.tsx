        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-emerald-50 border-2 border-emerald-200 rounded-xl p-4 text-center">
              ...
              <p className="text-sm font-bold text-emerald-600 mt-1">À jour ce mois</p>
              <p className="text-xs text-emerald-500 mt-0.5">{periodeActuelle}</p>
            </div>
            <div className="bg-red-50 border-2 border-red-200 rounded-xl p-4 text-center">
              <AlertCircle className="h-8 w-8 text-red-400 mx-auto mb-2" />
              <p className="text-3xl font-black text-red-600">{nbEnRetard}</p>
              <p className="text-sm font-bold text-red-500 mt-1">En retard</p>
              <p className="text-xs text-red-400 mt-0.5">n'ont pas payé {MOIS[MOIS_COURANT - 1]}</p>
            </div>
          ...
          {nbEnRetard > 0 && (
            <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
              <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
                <h3 className="text-sm font-bold text-red-600 flex items-center gap-2"><AlertCircle className="h-4 w-4" />Ménages en retard — {periodeActuelle}</h3>
                <button onClick={() => setSubMenu('cotisations')} className="text-xs text-green-600 font-semibold hover:underline">Gérer →</button>
              </div>
              <div className="divide-y divide-slate-50 max-h-48 overflow-y-auto">
                {foyers.filter(f => !foyersAJour.has(f.id)).slice(0, 10).map(f => {
                  const chef = membres.find(m => m.foyer_id === f.id && m.is_chef);
                  return (
                    <div key={f.id} className="flex items-center gap-3 px-4 py-2.5 hover:bg-red-50">
                      <span className="font-mono font-bold text-indigo-600 text-xs">{f.code_menage}</span>
                      <span className="text-sm text-slate-700">{chef ? `${chef.nom} ${chef.prenom}` : f.adresse || '—'}</span>
                      <span className="ml-auto text-xs text-red-500 font-semibold">Non payé</span>
                    </div>
                  );
                })}
          ...
          <div className="bg-white border border-slate-200 rounded-xl overflow-auto">
            <table className="w-full text-xs min-w-[900px]">
              ...
              <tbody className="divide-y divide-slate-100">
                {foyersCot.length === 0 ? (
                  <tr><td colSpan={14} className="text-center text-slate-400 py-8">Aucun foyer</td></tr>
                  ...
                  return (
                    <tr key={foyer.id} className={fi % 2 === 0 ? 'bg-white hover:bg-slate-50' : 'bg-slate-50/30 hover:bg-slate-50'}>
              ...
              <tbody className="divide-y divide-slate-50">
                {depPage.map(d => (
                  <tr key={d.id} className="hover:bg-slate-50">
                    ...
                    <td className="p-3 text-center"><button onClick={() => deleteDepense(d.id)} className="p-1.5 hover:bg-red-50 rounded-lg text-slate-300 hover:text-red-500 transition"><Trash2 className="h-4 w-4" /></button></td>
                  </tr>
                ))}
