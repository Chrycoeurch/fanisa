...
import { X, Home, MapPin, Image, Loader2, Upload, Building2, UserCog, Droplets, Trash2, Wifi, AlertTriangle, Info } from 'lucide-react';
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex border-b border-slate-200 overflow-x-auto shrink-0">
          {TABS.map(({ key, label, icon: Icon }) => {
            ...
            return (
              <button key={key} type="button" onClick={() => { if (idx <= curIdx) setTab(key as Tab); }} disabled={idx > curIdx}
                className={`flex items-center gap-1.5 px-3 py-3 text-xs font-semibold whitespace-nowrap border-b-2 transition shrink-0 ${isActive ? 'border-indigo-600 text-indigo-700' : isDone ? 'border-transparent text-emerald-600' : 'border-transparent text-slate-300 cursor-not-allowed'}`}>
